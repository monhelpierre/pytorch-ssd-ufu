...
class AveragePrecision(object):
    ...
    def update(self, det_boxes, det_scores, det_classes, true_boxes, true_classes,
        ...
        for i in range(bs):
            ...
            is_fn = np.full([num_dets, 10], True)
            ...
            if indices[0].shape[0] > 0:
                ...
                is_fp[is_tp] = False
                #is_fn[matches[:, 1][is_tp.flatten()].flatten()] = False

            self.true_positives.extend(is_tp.tolist())
            self.false_positives.extend(is_fp.tolist())
            self.false_negatives.extend(is_fn.tolist())
    ...
    def result(self):
        ...
        recalls = torch.zeros([self.num_classes, 10])
